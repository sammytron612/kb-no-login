<div>
    <div class="mb-4">
        <input type="text" wire:model.live.debounce.300ms="search" placeholder="Search articles..." class="w-full px-4 py-2 border rounded" />
    </div>
    <div class="grid gap-4">
        @if($articles)
             @forelse ($articles as $article)
                <div class="border shadow-lg rounded-md p-4 pb-4 flex flex-col">
                    <div class="flex items-center gap-2">
                        <a href="#" class="text-blue-500 text-lg font-semibold hover:underline">{{ $article->title }}</a>
            ...
            @empty
            <div class="mt-4">
                <span class="text-sm text-gray-600">Showing {{ $articles->firstItem() }} to {{ $articles->lastItem() }} of {{ $articles->total() }} articles</span>
            </div>
             @endforelse
    </div>
