                        ...
                        @can('canEdit', $article)
                            <div class="flex gap-2 ml-4">
                                ...
                                <form action="{{ route('articles.destroy', $article->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this article?');">
                                    ...
                                    @method('DELETE')
                                    <button type="submit"
                                        class="hover:cursor-pointer inline-flex items-center px-3 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600 hover:scale-105 hover:shadow-lg transition-all duration-200 text-sm font-medium">
                                        <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
